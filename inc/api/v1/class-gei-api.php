...
namespace GlobalEngineeringInitiative\Api_v1;
...
abstract class Api {
	...
	function apiErrors( $code ) {
		...
		switch ( $code ) {
			...
			case 'resource':
				$data = array(
				    'messages' => 'The API requires a valid resource in order to return a response. Try looking for \'resources\'',
				    'documentation' => '/api/v1/docs',
				);
				break;
			case 'version':
			...
			case 'offset':
				$data = array(
				    'messages' => 'The offset is a larger value than the number of resources available',
				    'documentation' => '/api/v1/docs',
				);
				break;
			default:
	...
	protected function renderJson( $data ) {
		if ( ! is_array( $data ) ) {
			throw new Exception( 'Data variable passed to \GlobalEngineeringInitiative\Api_v1\Api\renderJson is not an array' );
			$this->apiErrors( 'empty' );
		}
	...
	protected function renderXml( $data ) {
		if ( ! is_array( $data ) ) {
			throw new Exception( 'Data variable passed to \GlobalEngineeringInitiative\Api_v1\Api\renderXml is not an array' );
			$this->apiErrors( 'empty' );
		}
